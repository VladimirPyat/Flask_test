...
def date_local_format ():
    ...
    format = '%Y.%m.%d %H:%M'
    return  datetime.datetime.now(timezone).strftime('%Y.%m.%d %H:%M')
...
def log():
    ...
    if form.validate_on_submit():
        ...
        if form.login.data.strip() not in data:                                                        #проверка логин, убираем лишние пробелы
            return render_template('login.html', form=form, message='Вы не зарегистрированы')
        else:
            for i in data.split():
                if form.login.data.strip() in i:
